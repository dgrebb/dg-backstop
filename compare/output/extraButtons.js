...
const settingsHeader = document.querySelector(
  "section.header > div > section > div:nth-of-type(3)"
);
...
function addRefButton() {
  ...
  refButton.addEventListener("click", async function () {
    ...
    try {
      ...
      window.location.reload(true);
      console.error(
        "There was a problem requesting a reference run from Backstop Remote.",
        error
      );
    }
  });
...
async function pingUrl(url) {
  try {
    ...
    console.log(
      "%c The Backstop remote server no running! ",
